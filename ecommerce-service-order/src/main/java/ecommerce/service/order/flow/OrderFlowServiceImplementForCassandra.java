...
//import ecommerce.service.order.client.OrderException;
//import ecommerce.service.order.client.OrderFlowService;
//import ecommerce.service.order.dao.OrderDAOByCassandra;
//
///**
...
//	@Override
////	@Transactional
//	public Order createOrder(ShopCartOrder shopCartOrder) throws OrderException{
...
//		if (id==null) {
//			throw new OrderException(String.format("Failed to get main order: %d", shopCartOrder.getUserId()));
//		}
//
...
//
//		Result<List<Long>> batchResult = productService.increBatchQuantity(shopCartOrder.getProductIds(), shopCartOrder.getQuantities(), false);
//		if (batchResult==null||batchResult.getCode()!=Result.Code.OK.value) {
//			throw new OrderException(String.format("quantities not available: %d %s", shopCartOrder.getUserId(), shopCartOrder.getToken()));
//		}
//		try {
...
//					throw new OrderException(String.format("product not exist: %d", order.getProductId()));
//				}
//				Order subOrder = new Order();
//				Long subId = stringRedisTemplate.opsForValue().increment(KeyIdentifies.SubOrderId.value, 1);
//				if (subId==null) {
//					throw new OrderException(String.format("Failed to get sub order id: %d $d", order.getId(), shopCartOrder.getUserId()));
//				}
...
//					throw new OrderException(String.format("Failed to add sub order: %d %d %d %d", order.getId(), subId, shopCartOrder.getUserId(), shopCartOrder.getProductIds().get(i)));
//				}
//				order.setReceiptAmount(subOrder.getReceiptAmount()+order.getReceiptAmount());
//			}
//
//			if (!orderDAO.insert(order)) {
//				throw new OrderException(String.format("Failed to add main order: %d", id));
//			}
//		}catch(Exception e) {
//			Result<List<Long>> rollbackBatchResult = productService.increBatchQuantity(shopCartOrder.getProductIds(), shopCartOrder.getQuantities(), true);
//			if (rollbackBatchResult==null||rollbackBatchResult.getCode()!=Result.Code.OK.value) {
//				e.addSuppressed(new OrderException(String.format("quantities not available: %d %s", shopCartOrder.getUserId(), shopCartOrder.getToken())));
//			}
//			throw e;
//		}
...
//	@Override
////	@Transactional
//	public Order payment(Order order) throws OrderException {
//		if (order.getStatus()!=Order.Status.Actived.value) {
//			throw new OrderException(String.format("order status: %d %d", order.getId(), order.getStatus()));
//		}
//		if (order.getPaidAmount()==0) {
//			throw new OrderException(String.format("order payment amount not accepted: %d %d", order.getId(), order.getPaidAmount()));
//		}
//		if (!orderDAO.increPaidAmount(order.getId(), order.getPaidAmount())) {
//			throw new OrderException(String.format("Failed to add paid amount on main order: %d %d", order.getId(), order.getPaidAmount()));
//		}
//		
//		if (!orderDAO.updateStatus(order, Order.Status.Paid.value)) {
//			throw new OrderException(String.format("Failed to update status to Paid on main order: %d %d", order.getId(), order.getStatus()));
//		}
...
//				throw new OrderException(String.format("Failed to add paid amount on parent order: %d %d", order.getParentId(), order.getPaidAmount()));
//			}
//		}else {
...
//	@Override
////	@Transactional
//	public Order refund(Order order) throws OrderException {
...
//			throw new OrderException(String.format("Failed to update status to Refund on main order: %d %d", order.getId(), order.getStatus()));
//		}
...
//	@Override
////	@Transactional
//	public Order close(Order order) throws OrderException{
//		if (order.getParentId()>0&&order.getStatus()!=Order.Status.Actived.value) {
//			throw new OrderException(String.format("sub order status: %d %d", order.getId(), order.getStatus()));
//		}
//		if (order.getStatus()==Order.Status.Closed.value||order.getStatus()==Order.Status.Completed.value) {
//			throw new OrderException(String.format("order can't close: %d %d", order.getId(), order.getStatus()));
//		}
//		
//		if (!orderDAO.updateStatus(order, Order.Status.Closed.value)) {
//			throw new OrderException(String.format("Failed to update status to Closed on main order: %d %d", order.getId(), order.getStatus()));
//		}
...
//						throw new OrderException(String.format("Failed to update status to Closed on main order: %d %d", subOrder.getId(), subOrder.getStatus()));
//					}
...
//	@Override
////	@Transactional
//	public Order complete(Order order) throws OrderException{
//		if (order.getParentId()>0) {
//			throw new OrderException(String.format("sub order can't complete: %d", order.getId()));
//		}
//		if (order.getStatus()==Order.Status.Closed.value||order.getStatus()==Order.Status.Completed.value) {
//			throw new OrderException(String.format("order can't compelete: %d %d", order.getId(), order.getStatus()));
//		}
//		
//		if (!orderDAO.updateStatus(order, Order.Status.Completed.value)) {
//			throw new OrderException(String.format("Failed to update status to Completed on main order: %d %d", order.getId(), order.getStatus()));
//		}
...
//					throw new OrderException(String.format("Failed to update status to Completed on main order: %d %d", subOrder.getId(), subOrder.getStatus()));
//				}
